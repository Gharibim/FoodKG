...
import numpy as np
import pickle
from Prediction.helpers import io_helper
...
from Prediction.ml import loss_functions


source = 'Prediction/Glove/vectors.300d.txt'

def build_feed_dict_func(model, data, config = None, predict = False):
    print("Inssssssssssssssside")
    x_pairs, y = zip(*data)
...
def prepare_Models():
    ...
    print("Loading model...")
    print(os.getcwd())
    hyperparams, vars = pickle.load(open("Prediction/savedModel/AgroVoc/args.output", "rb"))
