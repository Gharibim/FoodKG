    ...
    def define_model(self, activation=tf.nn.tanh, previous_layer=None, share_params=None):
        ...
        with tf.variable_scope(self.scope + "__variables", reuse=share_params):
            for i in range(len(self.hidden_layer_sizes)):
                self.Ws.append(tf.get_variable("W_" + str(i),
                                               shape=[(self.input_size if i == 0 else self.hidden_layer_sizes[i - 1]),
                                                      self.hidden_layer_sizes[i]],
                                               initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float64))
                self.biases.append(tf.get_variable("b_" + str(i),
                                                   initializer=tf.constant(0.1, shape=[self.hidden_layer_sizes[i]],
        ...
        for i in range(len(self.Ws)):
            data_runner = tf.nn.dropout(activation(tf.nn.xw_plus_b(data_runner, self.Ws[i], self.biases[i])),
                                        self.dropout)
