...
parser = argparse.ArgumentParser(
    description='Predicts a lexico-semantic relation between pairs of words, using a previously trained classification model.')
parser.add_argument('data',
                    help='A path to the file containing prediction examples (test set). Each test example should have the format: "first_word TAB second_word". Pairs containing words that are not in the vocabulary of the input distributional embeddings will be skipped.')
...
model = wordpair_classifier.WordPairClassifier(embeddings, embedding_size, mlp_hidden_layer_sizes, same_mlp=same_mlp,
                                               bilinear_softmax=bilinear_softmax, num_mappings=num_mlps, activation=act,
                                               num_classes=len(dist_labels), noise_std=noise)
...
if args.preds is not None:
    ...
    to_write = list(zip([t_embeddings.get_word_from_index(x[0], lang="default") for x in predict_pairs],
                        [t_embeddings.get_word_from_index(x[1], lang="default") for x in predict_pairs], pred_labels))
