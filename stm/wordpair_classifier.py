...
class WordPairClassifier(object):
    ...
    ###	    -
    def __init__(self, embeddings, embedding_size, mlp_hidden_layer_sizes, same_mlp=True, bilinear_softmax=False,
                 num_mappings=1, activation=tf.nn.tanh, num_classes=2, noise_std=0.01, scope="word_pair_model",
                 dist_labels=None):
        ...
        self.embs_w1 = self.emb_layer.lookup(
            self.input_w1)  # tf.cond(self.training, lambda: noise.add_gaussian_noise_layer(self.emb_layer.lookup(self.input_w1), self.noise_std), lambda: self.emb_layer.lookup(self.input_w1))
        self.embs_w2 = self.emb_layer.lookup(
            self.input_w2)  # tf.cond(self.training, lambda: noise.add_gaussian_noise_layer(self.emb_layer.lookup(self.input_w2), self.noise_std), lambda: self.emb_layer.lookup(self.input_w2))
        ...
        for i in range(self.num_mlps):
            ...
            mlp_left = mlp_layer.MultiLayerPerceptron(mlp_hidden_layer_sizes, embedding_size,
                                                      scope="mlp_" + str(i) + ("" if same_mlp else "_first"),
                                                      unique_scope_addition="_1")
        ...
        self.right_mappers = []
        for i in range(self.num_mlps):
            mlp_right = mlp_layer.MultiLayerPerceptron(mlp_hidden_layer_sizes, embedding_size,
                                                       scope="mlp_" + str(i) + ("" if same_mlp else "_second"),
                                                       unique_scope_addition="_2")
        ...
        if self.bilinear_softmax:
            ...
            for i in range(self.num_mlps):
                ...
                with tf.variable_scope(self.scope + "__variables"):
                    bilinear_W = tf.get_variable("W_bilin_" + str(i), shape=[self.left_mappers[i].output_size,
                                                                             self.right_mappers[i].output_size],
                                                 initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float64)
                    bilinear_b = tf.get_variable("b_bilin_" + str(i),
                                                 initializer=tf.constant(0.1, shape=[1], dtype=tf.float64),
                                                 dtype=tf.float64)
                ...
                bilin_part = tf.add(tf.reduce_sum(tf.multiply(lin_part, self.right_mappers[i].outputs), axis=-1),
                                    bilinear_b)
            ...
            with tf.variable_scope(self.scope + "__variables"):
                self.W_class = tf.get_variable("W_class", shape=[self.num_mlps, self.num_classes],
                                               initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float64)
                self.b_class = tf.get_variable("b_class",
                                               initializer=tf.constant(0.1, shape=[self.num_classes], dtype=tf.float64),
                                               dtype=tf.float64)
        ...
        # VARIANT 2 of combining the specialized embeddinfs
            ...
            with tf.variable_scope(self.scope + "__variables"):
                self.W_class = tf.get_variable("W_class", shape=[self.mlp_hidden_layer_sizes[-1] * self.num_mlps * 2,
                                                                 self.num_classes],
                                               initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float64)
                self.b_class = tf.get_variable("b_class",
                                               initializer=tf.constant(0.1, shape=[self.num_classes], dtype=tf.float64),
        ...
        for i in range(self.num_mlps):
            self.l2_loss += self.left_mappers[i].l2_loss if same_mlp else (
                        self.left_mappers[i].l2_loss + self.right_mappers[i].l2_loss + self.right)
            if self.bilinear_softmax:
    ...
    def get_hyperparameters(self):
        return [self.same_mlp, self.bilinear_softmax, self.mlp_hidden_layer_sizes, self.num_mlps, self.num_classes,
                self.embedding_size, self.dist_labels]
